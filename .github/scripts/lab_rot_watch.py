...
def extract_urls(text: str) -> list[str]:
    return sorted({url.rstrip("`.,;:") for url in URL_PATTERN.findall(text)})
...
def check_external_links(
    ...
    skip_domains = {
        "localhost",
        "127.0.0.1",
        # Microsoft Entra resource audience, not a browsable website.
    ...
    url_list = [
        ...
        if not any(placeholder in url for placeholder in ("${", "<", ">", "..."))
    ]
    for url in url_list[:max_urls]:
        parsed = urllib.parse.urlparse(url)
        if not parsed.hostname or parsed.hostname in skip_domains:
            continue
        ...
        # Some sites reject or do not route HEAD correctly. Confirm a missing
        # response with GET before reporting it as learner-facing link rot.
...
def run(
    ...
    if not skip_external_links:
        findings.extend(check_external_links(markdown_files, max_urls=max_external_urls))
...
def main() -> int:
    ...
    parser.add_argument("--max-external-urls", type=int, default=100, help="Maximum external URLs to check")
